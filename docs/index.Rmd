...
```{r, echo=TRUE, eval=FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.width=6.5}
...
This function can take both aggregated data (`ho`) and user-separated (`ho_by_user`), but it does best with the latter. It returns metrics about hourly cough rates based on hour-by-hour analysis of the data. The argument `min_session` allows you to define the minimum amount of monitoring required during a single hour in order for that hour to be included in the cough rate estimation. For example, sometimes an hour of day contains only a few minutes of monitoring for a user; that makes for a pretty poor estimate of that hour's cough rate. The default `min_session` is 0.5 hours, or 30 minutes of monitoring within an hour.  
...
Examine how many coughs were contaiend in these bouts:  
