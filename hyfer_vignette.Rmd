...
(1) Download Hyfe data. Researchers do so from the [Hyfe Research Dashboard](https://dashboard.hyfe.ai/). Hyfe employeers do so using `hyferdrive`. Anyone can do so using the sample dataset that comes with `hyfer`:
...
```{r, echo=FALSE, eval=TRUE, include=FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.width=6.5}
# Daily timeseries of coughs (all users)
ggplot(ho$days,aes(x=date, y=coughs)) + geom_col()

# Daily timeseries of coughs (just one user)
ho1 <- ho_users$user_summaries[[2]]
ggplot(ho1$days,aes(x=date, y=coughs)) + geom_col()

# Daily timeseries of hourly cough rate
ggplot(ho1$hours,aes(x=study_hour, y=cough_rate)) + geom_point()

# Plot circadian pattern in user cough
ggplot(ho1$hours,aes(x=hour,y=cough_rate)) + geom_jitter(width=.3) 

# Plot distribution of hourly cough rate
...
Hyfe's research collaborators can download data for their respective research cohorts from the [Hyfe Research Dashboard](https://dashboard.hyfe.ai/).  

Hyfe's internal analysts download data directly using `hyferdrive`, a private company package. Both the dashboard and `hyferdrive` deliver data structured in exactly the same way, allowing both groups to utilize the functions offered in `hyfer`.  
...
```{r, echo=TRUE, eval=TRUE, collapse=TRUE, warning=FALSE, message=FALSE, fig.height=6.5}
hyfe_time <- expand_sessions(hyfe_data,
                             create_table = FALSE,
                             create_series = TRUE,
                             inactive_value = NA)
...
This function is essentially a wrapper for `expand_sessions()`, and calls both that function and `format_hyfe_time()`.  
...
- Function for getting cumulative totals  
